...
- Various controller emulation modes
  ...
  - PS4 Compatibility (will work on PS4, see [PS4 Authentication](#ps4-authentication) for details)
  - Dualshock 4 (Will be recognised as DS4 on PC/Steam, but does not work on an actual PS4!)
  - Dualshock 3
...
| Hatsune Miku -Project DIVA- Future Tone                                     |   PS4    |               No               |         PS4 Compat         | - See [PS4 Authentication](#ps4-authentication)<br>- Game does not support arcade style slider, stick emulation will be used.                                                                                                                                                                                                                        |
| Hatsune Miku -Project DIVA- Future Tone DX                                  |   PS4    |              Yes               |          PS4 Diva          | - See [PS4 Authentication](#ps4-authentication)<br>- From song selection enter the 'Customize' menu and enable arcade controller support under 'Game/Control Config' -> 'Arcade Controller Settings'.                                                                                                                                                |
| Hatsune Miku -Project DIVA- X                                               |   PS4    |               No               | PS4 Compat + 'Double Btn'  | - See [PS4 Authentication](#ps4-authentication)<br>- Slider can be used for Star notes.<br>- Enable 'Double Btn' to make hitting 'W'-arrows easier.                                                                                                                                                                                                  |
...
The PS4 needs a controller to sign a cryptographic challenge every few seconds, otherwise it will stop working after around 8 minutes after plugging in. For the Project Diva games this is somewhat bearable, since you can re-plug the controller before starting each song to avoid running into the timeout during gameplay. Still, this is annoying.

DivaCon2040 can sign those challenges, but you will need to obtain some data from an original DS4 and compile it into the firmware. You will need a serial, signature and private key file. I can't and won't help you how to obtain those, you'll need to figure this out yourself.
...
Signing the challenge will block the second core of the rp2040 for 2-3 seconds, so LED animations and the display will appear stuck from time to time. Input handling is unaffected.
